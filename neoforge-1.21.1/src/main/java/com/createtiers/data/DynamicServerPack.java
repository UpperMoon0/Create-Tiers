...
/** Dynamic NeoForge 1.21.1 server data for registered tier components. */
public class DynamicServerPack implements PackResources {

    private static final String NAME = "createtiers:dynamic_server";
    private static final String BLOCK_TAG_ROOT = "tags/block";
    private static final String LOOT_TABLE_ROOT = "loot_table";
    ...
    private static void generateBlockTags() {
        ...
        TAGS.put(Compat.rl("minecraft", BLOCK_TAG_ROOT + "/mineable/axe"), mineableAxe);
    ...
    private static void generateDefaultTierUpgradeRecipes() {
        for (TierUpgradeRegistry.Registration upgrade : TierUpgradeRegistry.getAll()) {
            if (!upgrade.defaultRecipe()) continue;
            ...
            String path = RECIPE_ROOT + "/tier_upgrade/"
    ...
    private static void generateBlockLootTable(String blockName) {
        LOOT_TABLES.put(
                Compat.rl(CreateTiers.MOD_ID, LOOT_TABLE_ROOT + "/blocks/" + blockName),
    ...
    private static void generateEncasedBlockLootTable(String encasedBlockName, String dropBlockName) {
        LOOT_TABLES.put(
                Compat.rl(CreateTiers.MOD_ID, LOOT_TABLE_ROOT + "/blocks/" + encasedBlockName),
    ...
    public @Nullable IoSupplier<InputStream> getResource(@NotNull PackType type, @NotNull ResourceLocation location) {
        ...
        if (path.startsWith(BLOCK_TAG_ROOT + "/") && path.endsWith(".json")) {
            JsonObject json = TAGS.get(Compat.rl(namespace, path.substring(0, path.length() - 5)));
            return json == null ? null : () -> stream(json);
        }
        if (namespace.equals(CreateTiers.MOD_ID) && path.startsWith(LOOT_TABLE_ROOT + "/") && path.endsWith(".json")) {
            JsonObject json = LOOT_TABLES.get(Compat.rl(namespace, path.substring(0, path.length() - 5)));
            return json == null ? null : () -> stream(json);
        }
        if (namespace.equals(CreateTiers.MOD_ID) && path.startsWith(RECIPE_ROOT + "/") && path.endsWith(".json")) {
    ...
    public void listResources(@NotNull PackType type, @NotNull String namespace, @NotNull String path,
        ...
        if (path.startsWith(BLOCK_TAG_ROOT + "/") || path.equals(BLOCK_TAG_ROOT) || path.equals("tags")) {
        ...
        if (namespace.equals(CreateTiers.MOD_ID)
                && (path.startsWith(LOOT_TABLE_ROOT + "/") || path.equals(LOOT_TABLE_ROOT))) {
        ...
        if (namespace.equals(CreateTiers.MOD_ID)
                && (path.startsWith(RECIPE_ROOT + "/") || path.equals(RECIPE_ROOT))) {
